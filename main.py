...
parser.add_argument(
    ...
    default=128,
    choices=[128, 192, 256],
    help="Kyber security level in bits (128, 192, or 256).",
)
...
if args.kyber_level == 128:
    ...
    print(
        f"Warning: Invalid Kyber level {args.kyber_level}. Defaulting to Kyber512 (128 bits)."
    )
    kyber = Kyber512()
    kyber_bit_label = 128
...
print(
    f"[RESULT] RSA ({args.rsa_bits} bits) time: {rsa_end_time - rsa_start_time:.6f} seconds"
)
print(
    f"[RESULT] Kyber ({kyber_bit_label} bits) time: {kyber_end_time - kyber_start_time:.6f} seconds"
)
